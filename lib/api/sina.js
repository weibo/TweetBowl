/**
 * This jQuery plugin is about sina api.
 * 
...
(function($){
	...
	$.sina = {
		...
		config		: {count: 20, source: '3387893565',},
		statuses : {},
		results  : {}
	}
	...
	$.sina.results.verify = function(userInfo,options){
		...
		if (userInfo.status) {
			var value = userInfo.status;
			if (value.retweeted_status && value.retweeted_status.text && value.retweeted_status.user) {
				value.text = value.text + "<br/>@[url=http://t.sina.com.cn/" + value.retweeted_status.user.id + "]" + value.retweeted_status.user.name + "[/url]:" + value.retweeted_status.text;
			}
		}
	...
	$.sina.results.friends_timeline = function(data,options) {
		...
		if(data && data.length) {
			...
			$.each(data, function(index,value){
				if(value.retweeted_status && value.retweeted_status.text && value.retweeted_status.user) {
					value.text = value.text + "@[url=http://t.sina.com.cn/" + value.retweeted_status.user.id + "]" + value.retweeted_status.user.name + "[/url]:" + value.retweeted_status.text;
				}
			});
