(function($){
	...
	$.tongxue.friends = function(params, successCallback) {		
		...
		var authz = $.base64.encode($.tongxue.username + ':' + $.tongxue.password);		
		Ext.Ajax.request({
			url: $.tongxue.urls.users_friends + '.json',
	...
	$.tongxue.success = function(response,options) {
	...
	$.tongxue.results.verify = function(response,options){
		$.message.hideLoadMask();
	...
	$.tongxue.results.friends = function(response,options) {
		$.message.hideLoadMask();
		//alert(response.responseText);
		var data = Ext.decode(response.responseText);
		if(data.length) {
			$.tongxue.results.tweetcache.friends = data;
			if($.tongxue.results.callback) {
				$.tongxue.results.callback($.tongxue.results.tweetcache.friends);
			}
		}
	};
})(jQuery);